...
localparam logic [5:0] V_FUNCT6_LOAD_DENSE          = 6'b000001;    // opcode下的Load功能
...
localparam logic [5:0] V_FUNCT6_LOAD_SPARSE         = 6'b000100;    // opcode下的spmm load功能
...
localparam logic [5:0] V_FUNCT6_LOAD_TMP            = 6'b000111; // opcode下的spmm load vector功能 todo
...
always_ff @(posedge clk or negedge rstn) begin
    if(~rstn) begin
        ...
        if(load_index_ready_i && opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_LOAD_DENSE) begin
...
always_ff @(posedge clk or negedge rstn ) begin
    if (~rstn) begin
        ...
        if ( vlsu_done_i && (state == HOLD) && ((opcode == V_OPCODE_SPMM) && (funct6 == V_FUNCT6_LOAD_DENSE) || (pe_op == PE_SPMM_FIX_ROW))) begin
            number_rows <= number_rows - 1'b1;
        end else if ((state == EXEC) && ((load_index_ready_i && (opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_LOAD_DENSE)) || (pe_op == PE_SPMM_FIX_ROW))) begin  //这一块只能赋值一次 也就是在exec的时候 后面进入了hold 开始用递减逻辑
...
//传给vex的spmm_load完成信号
assign spmm_load_done = ((state == HOLD) && vlsu_done_i && (number_rows == 1)) || number_rows == 0 && (state == HOLD) && funct6 == V_FUNCT6_LOAD_DENSE; //number_rows==0说明只有一行
...
always_ff @(posedge clk or negedge rstn) begin
    ...
    end else if (dense_buf_read_en_o && funct6 == V_FUNCT6_LOAD_TMP && opcode == V_OPCODE_SPMM) begin
    ...
    if (~rstn) begin
        ...
        if (next_state != HOLD) begin
            ...
            if ((reg_instr[6:0] == V_OPCODE_SPMM) && 
                (reg_instr[30:25] == V_FUNCT6_LOAD_DENSE) && 
...
if ((load_index_ready_r && (opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_LOAD_DENSE)) || pe_op == PE_SPMM_FIX_ROW) begin
    ...
    case ( state )
        ...
        begin
            if (vlsu_en_o | (vex_en & multi_cycle_instr & funct6 != V_FUNCT6_COMPUTE) | dma_instr_valid_o |  (~extra_load_valid & (funct6 == V_FUNCT6_COMPUTE && opcode == V_OPCODE_SPMM) && (compute_row_data_num != 0) ))begin //如果出现vlsu或多周期vex指令，进入 HOLD 状态
                ...
                hold_req_o <= 4'b0001;
                core_activate <= 1'b1;
            end else if (dense_buf_read_en_o_tag && pe_op == PE_SPMM_LOAD_TMP) begin
                next_state <= HOLD;
                hold_req_o <= 4'b0001;
                core_activate <= 1'b1;
                ...
                next_state <= EXEC;
                core_activate <= 1'b0;
            end else if(funct6 == V_FUNCT6_LOAD_DENSE) begin
                hold_req_o <= 4'b0001;
                next_state <= EXEC;
                core_activate <= 1'b0;  
            end else if(number_rows == 0 && funct6 == V_FUNCT6_LOAD_DENSE) begin
                hold_req_o <= 4'b0000;
                next_state <= WAIT;
        ...
        begin
            if(vlsu_done_i && number_rows == 1 && funct6 == V_FUNCT6_FIX_ROW && opcode == V_OPCODE_SPMM) begin  //load指令当中 load完成的话则vlsu会发送vlsu_done_i的信号
            ...
            else if( funct6 == V_FUNCT6_LOAD_DENSE && vlsu_done_i && number_rows == 1 || (funct6 == V_FUNCT6_LOAD_DENSE && number_rows == 0)) begin
                ...
                hold_req_o <= 4'b0000;
                core_activate <= 1'b1;
            end else if((funct6 == V_FUNCT6_LOAD_SPARSE && dma_store_done) || index_cal_done) begin 
                ...
                hold_req_o <= 4'b0000;
                core_activate <= 1'b1;
            end else if ((opcode == V_OPCODE_SPMM) & (funct6 == V_FUNCT6_LOAD_TMP) & (load_vector_count == 3'b010) )begin //三次load之后跳转到exec接收下一个指令
                next_state <= EXEC;
                hold_req_o <= 4'b0000;
                core_activate <= 1'b1;
...
always_ff @(posedge clk or negedge rstn) begin
    if (~rstn) begin
        ...
        vd_addr_r <= 'b0;
            ...
            else if (dense_buf_read_en_o_tag &&  funct6 == V_FUNCT6_LOAD_TMP && opcode == V_OPCODE_SPMM )begin
                vd_addr_r <= '0;  //清零逻辑
            end
            else if (dense_buf_read_en_o && funct6 == V_FUNCT6_LOAD_TMP && opcode == V_OPCODE_SPMM) begin
...
always_ff @(posedge clk or negedge rstn) begin
    if (~rstn) begin
        ...
        if (opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_STORE) begin
        ...
        end else if(opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_LOAD_DENSE) begin
            dense_buf_data_source_o = DENSE_WB_SRC_VLSU;
            ...
            vs3_addr_src = VS3_ADDR_SRC_DECODE;        //地址从rd来
        end  else if (opcode == V_OPCODE_SPMM && funct6 == V_FUNCT6_LOAD_TMP) begin 
        ...
        case (opcode)
            V_OPCODE_SPMM:begin
                case (funct6)
                    ...
                    V_FUNCT6_LOAD_DENSE: begin // SPMM Load
                        dma_instr_valid_o = 1'b0;
                    ...
                    V_FUNCT6_LOAD_SPARSE: begin    //通过dma往buffer里存稀疏矩阵数据,最一开始执行一次，后面不再执行
                        multi_cycle_instr = 1'b1;
                        vex_en = 1'b0; // 不需要vex
                        dma_store_en = 1'b0;
                        if(dma_store_done) begin
                    ...
                    V_FUNCT6_LOAD_TMP: begin
                        pe_op = PE_SPMM_LOAD_TMP;
                        dma_instr_valid_o = 1'b0;
                        multi_cycle_instr = 1'b1;
                        pe_op = PE_SPMM_LOAD_TMP;
    ...
    if (state == HOLD) begin   //HOLD时保持enable
        ...
        V_OPCODE_SPMM:begin
            ...
            V_FUNCT6_LOAD_DENSE: begin //spmm load
                ...
                sparse_load = 1'b1;
                dense_buf_write_addr_o = vd_addr_r;
                pe_op = PE_SPMM_LOAD_DENSE; // SPMM加载操作
